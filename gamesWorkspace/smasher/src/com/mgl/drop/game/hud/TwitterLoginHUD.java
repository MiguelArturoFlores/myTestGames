...
public class TwitterLoginHUD extends HUD {
	...
	public TwitterLoginHUD() {
		try {
			...
			Text quantity = ObjectFactorySingleton.getInstance().createText("x10", TextureSingleton.getInstance().getmFont2());
			quantity.setPosition(diamant.getX()+diamant.getWidth(), diamant.getY()+diamant.getHeight()/2 - quantity.getHeight()/2);
