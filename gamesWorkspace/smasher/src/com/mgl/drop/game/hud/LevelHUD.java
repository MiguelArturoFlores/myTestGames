...
public class LevelHUD extends HUD {
	...
	public LevelHUD(Scene scene, LevelManager levelManager) {
		try {
			...
			SpriteBackground heart =  new SpriteBackground(0, 0, texture.getTextureByName("heart.png"), texture.getVertexBufferObjectManager());
			heart.setSize(40, 40);
			heart.setPosition(liveText.getX()-3-heart.getWidth(), 0);
			
			scoreText = ObjectFactorySingleton.getInstance().createText( "" , texture.getmFont1());
			scoreText.setPosition(camera.getWidth()-350,0);

			Sprite waveImg = new Sprite(0,0,
					texture.getTextureByName("wave.png"),
					texture.getVertexBufferObjectManager());
			waveImg.setSize(50, waveText.getHeight());
			waveImg.setPosition(waveText.getX()-3 - waveImg.getWidth() ,waveText.getY());
