...
public class SceneManagerSingleton {
	...
	public enum AllScenes {
		SPLASH, MAIN, SELECT_LEVEL, GAME, GAME_BEGIN, MODE_SELECT, MONEY_SHOP, HELP, SCORE, TROPHY, WIN_GAME
	}
	...
	public void setCurrentScene(AllScenes currentScene) {
		try {
			...
			case GAME_BEGIN:
				...
				SoundSingleton.getInstance().playMusic(
						"backgroundSound1.mp3",true);
				if (gameScene != null) {
	...
	public void goBack() {
		try {
			...
			if(sc.equals(AllScenes.HELP) || sc.equals(AllScenes.TROPHY) || sc.equals(AllScenes.MODE_SELECT) || sc.equals(AllScenes.WIN_GAME) || sc.equals(AllScenes.SCORE)){
				setCurrentScene(AllScenes.MAIN);
			}else if(sc.equals(AllScenes.MONEY_SHOP)){
				if (!sceneStack.isEmpty()) {
					setCurrentScene(sceneStack.pop());
				}
			}else if(sc.equals(AllScenes.GAME_BEGIN) || sc.equals(AllScenes.SELECT_LEVEL) ){
				setCurrentScene(AllScenes.MODE_SELECT);
			}
