...
public class WebServiceSingleton {
	...
	*/
	...
	private final String URL = "http://173.254.28.135/~idontrem/webServices/serverIdrGamesVampire.php";
	private final String SOAP_ACTION = "http://173.254.28.135/~idontrem/webServices/insertDataUserVampire";
	private final String METHOD_NAME = "insertDataUserVampire";
	private String TAG = "PGGURU";
	...
	private class SendEmail extends AsyncTask<String, Void, Void> {
		...
		protected Void doInBackground(String... params) {
			try {
				...
				if (result) {
					// HUDManagerSingleton.getInstance().addHUD(new
					// InformativeHUD("trueACA " +result), true);
					UserInfoSingleton.getInstance().setHasSendMail(true);
				} else {
					 //HUDManagerSingleton.getInstance().addHUD(new
					 //InformativeHUD("falseACA "+result), true);
