...
public class InformativeHUD extends HUD {
	...
	public InformativeHUD(String textToShow) {
		try {
			...
			loadInformativeHUD(textToShow, true);
	...
	private void loadInformativeHUD(String textToShow, boolean removable) {
		try {
			...
			for (int i = 0; i < split; i++) {
				...
				if ((i * quantity) + quantity > textToShow.length()) {
					textAux = textToShow.substring(i * quantity,
							textToShow.length() + 1);
				} else {
					textAux = textToShow.substring(i * quantity, (i * quantity)
							+ quantity);
				}
	...
	public InformativeHUD(String textToShow, boolean removable) {
		try {

			loadInformativeHUD(textToShow, removable);
