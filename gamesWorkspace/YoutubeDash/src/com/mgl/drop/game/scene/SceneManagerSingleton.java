...
public class SceneManagerSingleton {
	...
	public enum AllScenes {
		SPLASH, MAIN, SELECT_LEVEL, GAME, GAME_BEGIN, MODE_SELECT, MONEY_SHOP, HELP, SCORE, TROPHY, WIN_GAME, POWER_UP, INTRO, SELECT_PLAYER
	}
	...
	public void setCurrentScene(AllScenes currentScene) {
		try {
			...
			case SELECT_PLAYER:
				...
				SoundSingleton.getInstance().playMusic("music.ogg", false);
				contRetry = 0;
				ScenePlayerSelect playerSelect = new ScenePlayerSelect();
				engine.setScene(playerSelect);
				currentSceneTrue = playerSelect;

				getActivity().requestInventoryItems();
				//WebServiceSingleton.getInstance().sendDataAsync();
				
				
				//GoogleAPISingleton.getInstance().loginToYoutube();
				...
				//HUDManagerSingleton.getInstance().addHUD(new InformativeHUD("A"+UserInfoSingleton.getInstance().getEmail()+"A"), true);
			...
			case GAME_BEGIN:
				...
				if (gameScene != null) {
					...
					// SoundSingleton.getInstance().getMusic("music.ogg")
					// .seekTo(0);
					gameScene.loadLevel();
	...
	public void goBack() {
		try {
			...
			if (sc.equals(AllScenes.MAIN)) {
				...
				setCurrentScene(AllScenes.SELECT_PLAYER);
			}
			else if (sc.equals(AllScenes.SELECT_PLAYER)) {
				setCurrentScene(AllScenes.MAIN);
			}
