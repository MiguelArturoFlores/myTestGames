...
public final class FacebookManager {
	...
	private static Session openActiveSession(final Activity pActivity,
			final boolean pAllowLoginUI, final StatusCallback pCallback,
			final List<String> pPermissions) {
		final OpenRequest openRequest = new OpenRequest(pActivity)
				.setPermissions(pPermissions).setCallback(pCallback);
		openRequest.setLoginBehavior(SessionLoginBehavior.SUPPRESS_SSO);
		final Session session = new Session.Builder(pActivity.getBaseContext())
				.build();
	...
	public static void performAction(String action, Level level) {
		try {
			...
			if(action.equals(SHARE_VOLUNTARY)){
				...
				t.setScreenName("Share Voluntary");
				t.send(new HitBuilders.AppViewBuilder().build());

			}else if(action.equals(SHARE_TO_UNLOCK)){
				Tracker t = SceneManagerSingleton.getInstance().getActivity().getTracker(TrackerName.APP_TRACKER);
				t.setScreenName("Share To Unlock "+level.getId());
				t.send(new HitBuilders.AppViewBuilder().build());
				shareToUnlock(level);
			}
	...
	private static void shareToUnlock(Level level) {
		...
		try {
			...
			LevelDAO dao = new LevelDAO(SceneManagerSingleton.getInstance().getActivity(), MainDropActivity.DB_NAME, null, MainDropActivity.DB_VERSION);
		...
		yourActivity.runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				params.putString(
						...
						//"http://3.bp.blogspot.com/-MUEzPzbr7aI/VB8vPYJoaRI/AAAAAAAAAEc/-U5QT3Nskto/s1600/backgroundBegin.png");
						//"http://2.bp.blogspot.com/-7SUbbuakF2U/VBtzBh1MWlI/AAAAAAAAAEM/IfHLCRnItw4/s1600/FONDO%2BPROXIMAMENTE.png");
						//"https://lh5.ggpht.com/65OT4Sm9Iru56RbJ1Dmf2XcIKTb32I1BOfen8DdC3WEeu_jxckAmXqfk08Q6b9B85Qs=h310");
