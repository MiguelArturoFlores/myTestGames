...
public class PauseHUD extends HUD {
	...
	public PauseHUD(Scene scene, LevelManager levelManager) {
		try {
			...
			spriteBack.setSize(100, camera.getHeight());
			
			//spriteBack.setAlpha(0.9f);
			spriteBack.registerEntityModifier(new MoveModifier(0.2f, -100, 0,
					0, 0));

			Sprite background = new Sprite(100, 0,
					texture.getTextureByName("black.jpg"),
					texture.getVertexBufferObjectManager());
			background.setAlpha(0.4f);
			background.setSize(camera.getWidth(), camera.getHeight() * 2);
			...
			ContinueButton continueButton = new ContinueButton(0, 0,
					texture.getTextureByName("startOrange.png"),
			...
			DisableMusicButton disable = new DisableMusicButton(camera.getWidth() - 240, 20,
					texture.getTextureByName("orangeVolumeOff.png"),
					texture.getVertexBufferObjectManager(), scene, this); 
			disable.setSize(60, 60);
			disable.setPosition(20,selectLevelButton.getY()+60+40);
			
			EnableMusicButton enable = new EnableMusicButton(camera.getWidth() - 240, 20,
					texture.getTextureByName("orangeVolumeOn.png"),
					texture.getVertexBufferObjectManager(), scene, this); 
			enable.setSize(60, 60);
			enable.setPosition(20,camera.getHeight()-240-continueButton.getHeight() -offset);
