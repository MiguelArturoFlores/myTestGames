...
public class SpriteMonsterFat extends MyAnimateSprite{
	...
	public SpriteMonsterFat(float pX, float pY,
			ITextureRegion pTextureRegion,
			VertexBufferObjectManager pVertexBufferObjectManager, LevelController level,float minX,float maxX) {
	...
	private void initSoundHash() {
		try {
			...
			for (int i =1; i <=5; i++){
				normalSoundHash.put(Integer.valueOf(i), "fatNormal"+i+".mp3");
			}
	...
	public void poop(MySpriteGeneral poop,LevelController controller) {
		try {
			if(poop instanceof SpritePoopRocket){
				...
				SpriteBigShit spr = new SpriteBigShit(0, 0, TextureSingleton.getInstance().getTextureAnimateByName("bigShit.png"),TextureSingleton.getInstance().getVertexBufferObjectManager(), level);
	...
	public void initHashMap() {
		...
		stateAnimate.put(State.WALKIN_RIGHT, new MyAnimateProperty(3, 3,new long[] { 100, 100 , 100}));
		stateAnimate.put(State.WALKIN_LEFT, new MyAnimateProperty(0, 3,new long[] { 100, 100 , 100} ));
		stateAnimate.put(State.POOP_BEGIN, new MyAnimateProperty(6, 3,new long[] { 200, 300, 200 }));
		stateAnimate.put(State.POOP_END, new MyAnimateProperty(6, 3,new long[] { 200, 200 , 200}));
		
		
		stateAnimate.put(State.WALKIN_DOWN, new MyAnimateProperty(0, 2,new long[] { 200, 200 } ));
		stateAnimate.put(State.WALKIN_UP, new MyAnimateProperty(0, 2,new long[] { 200, 200 }));
