...
public class SpriteMonsterJustin extends MyAnimateSprite {
	...
	public SpriteMonsterJustin(float pX, float pY,
	...
	private void initSoundHash() {
		try {
			...
			for (int i =1; i <=2; i++){
				poopSoundHash.put(Integer.valueOf(i), "justinPoop"+i+".mp3");
			}
		...
		try {
			...
			if (status.equals(StatusType.NORMAL)) {
				...
				if (contTwerking >= maxNormal) {
					...
					ThreadSoundOnce thread = new ThreadSoundOnce("justinNormal.mp3", 0f);
					thread.start();
	...
	public void initHashMap() {
		...
		stateAnimate.put(State.WALKIN_RIGHT, new MyAnimateProperty(0, 10,
				new long[] { 100, 100, 100, 100, 100, 100, 100, 100,100,100 }));
		stateAnimate.put(State.WALKIN_LEFT, new MyAnimateProperty(10, 10,
				new long[] { 100, 100, 100, 100, 100, 100, 100, 100,100,100 }));
		stateAnimate.put(State.TWERKING_RIGHT, new MyAnimateProperty(20, 4,
				new long[] { 400, 400,400,400 }));
		stateAnimate.put(State.TWERKING_LEFT, new MyAnimateProperty(30, 4,
				new long[] { 400, 400,400,400 }));

		stateAnimate.put(State.POOP_BEGIN, new MyAnimateProperty(40, 6,
				new long[] { 200, 200, 200,200, 200, 200 }));
		stateAnimate.put(State.POOP_END, new MyAnimateProperty(40, 6,
				new long[] { 200, 200, 200,200, 200, 200 }));
