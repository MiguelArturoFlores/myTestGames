...
public class SelectLevelScene extends Scene {
	...
	private SceneMoveListener moveListener;
	...
	public Scene createScene() {
		try {
			...
			Sprite background = new Sprite(0, 0, texture.getTextureByName("backgroundLevel.png"),texture.getVertexBufferObjectManager());
			...
			// moveListener = new SceneMoveListener(CAMERA_MAX_WIDTH,
			// CAMERA_MAX_HEIGHT);
			// registering touch areas

			// this.setOnSceneTouchListener(moveListener);
			
			ButtonShareFacebook bS = new ButtonShareFacebook(MainDropActivity.CAMERA_WIDTH - 80, 10 ,70,70,texture.getTextureByName("shareYellow.png"),texture.getVertexBufferObjectManager(),null);
			//this.attachChild(bS);
			//this.registerTouchArea(bS);

			ButtonGoBeginGame sprite = new ButtonGoBeginGame (5, 200,
					texture.getTextureByName("back.png"),
	...
	private void initNavigationButtons() {
		try {
			...
			SpriteNavigateNext sprite = new SpriteNavigateNext(5, 200,
			...
			SpriteNavigateNext sprite2 = new SpriteNavigateNext(750, 200,
	...
	private void addLevelImage(Level level, Long px, Long py) {
		try {
			...
			SpriteSelectLevel selectSprite = new SpriteSelectLevel(px, py,
					TextureSingleton.getInstance().getTextureByName(textureName),
					TextureSingleton.getInstance()
							.getVertexBufferObjectManager(), level, levelList);
			...
			Sprite star = new Sprite(0, 0, texture.getTextureByName("emptyStar.png"), texture.getVertexBufferObjectManager());
			star.setSize(23, 27);
			star.setPosition(5,selectSprite.getHeight() +yStar);
			selectSprite.attachChild(star);
			
			star = new Sprite(0, 0, texture.getTextureByName("emptyStar.png"), texture.getVertexBufferObjectManager());
			star.setSize(23, 27);
			star.setPosition(10+star.getWidth(),selectSprite.getHeight() + yStar);
			selectSprite.attachChild(star);
			
			star = new Sprite(0, 0, texture.getTextureByName("emptyStar.png"), texture.getVertexBufferObjectManager());
			star.setSize(23, 27);
			star.setPosition(15+star.getWidth()*2,selectSprite.getHeight() +yStar);
			selectSprite.attachChild(star);
			...
			textName.setPosition(selectSprite.getWidth()/2 -textName.getWidth()/2, selectSprite.getHeight()/2 -textName.getHeight()/2);
			
			selectSprite.attachChild(textName);
			
			
			 for(int i=0;i<selectSprite.getLevelDB().getStars() ; i++){
				 star = new Sprite(0, 0, texture.getTextureByName("fillStar.png"), texture.getVertexBufferObjectManager());
				 star.setSize(23, 27);
				 star.setPosition(5+(i*5)+star.getWidth()*(i),selectSprite.getHeight() +yStar);
				 selectSprite.attachChild(star);
			 }
		...
		//	selectSprite.attachChild(textStars);
			...
			if (level.getAvalible()) {
				...
				ButtonShowUnlockLevelHUD stu = new ButtonShowUnlockLevelHUD(0, 0, texture.getTextureByName("unlockFacebook.png"), texture.getVertexBufferObjectManager(), level); 
				stu.setSize(50, 50);
				stu.setPosition(0,0);
	...
	/*private String readFromFile() {
		try {
			...
			String displayText = "";
			InputStream fileStream = sceneManager.getActivity().getResources()
...
*/
	...
	public SceneMoveListener getMoveListener() {
		return moveListener;
	}

	public void setMoveListener(SceneMoveListener moveListener) {
