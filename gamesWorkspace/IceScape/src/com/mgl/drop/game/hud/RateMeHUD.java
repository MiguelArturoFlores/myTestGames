...
public class RateMeHUD extends MyHud {
	...
	public RateMeHUD() {
		try {
			...
			Text text = ObjectFactorySingleton.getInstance().createText(
					SceneManagerSingleton.getInstance().getActivity().getString(R.string.RATE_MESSAGE), TextureSingleton.getInstance().getmFont2());
			...
			Text text1 = ObjectFactorySingleton.getInstance().createText(
					...
					.getActivity()
					.getString(
							R.string.PLEASE_RATE), TextureSingleton.getInstance().getmFont2());
			text1.setPosition(MainDropActivity.CAMERA_WIDTH/2 - text1.getWidth()/2, text.getY()+ text.getHeight()+50);
	...
	private void addFiveStars() {
		try {
			...
			rate1.setPosition(25,  MainDropActivity.CAMERA_HEIGHT/2 - rate1.getHeight()/2);
