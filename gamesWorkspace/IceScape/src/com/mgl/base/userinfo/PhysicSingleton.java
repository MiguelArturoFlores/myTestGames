	...
	private PhysicSingleton() {
		...
		try {
			...
			physicsWorld = new PausablePhysicWorld(new Vector2(0,
					SensorManager.GRAVITY_PLUTO), true);
				...
				if (sprite instanceof MyAnimateSprite) {
					...
					if (sprite.getCollitionType() == CollitionType.COLLITION_CIRCULAR) {
						...
						body = PhysicsFactory.createBoxBody(physicsWorld, spr,
								bodyType, fixtureDef);
					} else if (sprite.getCollitionType() == CollitionType.COLLITION_PLAYER) {
						
						float x = spr.getWidth()/(2*32);
						float y = spr.getHeight()/(2*32);
						
						Vector2[] vetrices = {new Vector2(x*0.6f* -1, y), new Vector2(x*0.6f * -1,0), new Vector2(x*0.6f ,0),  new Vector2(x*0.6f,y)};
						body = PhysicsFactory.createPolygonBody(physicsWorld, spr, vetrices, bodyType, fixtureDef);
					}
