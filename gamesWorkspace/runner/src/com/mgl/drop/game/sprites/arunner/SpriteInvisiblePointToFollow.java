...
public class SpriteInvisiblePointToFollow extends MySprite {
	...
	public void update(float dTime, LevelController level) {
		...
		try {
			...
			this.setX(playerX);this.setY(playerY);
		...
		try {
			...
			for(MySpriteGeneral spr : level.getSpriteList()){
				if(spr instanceof MySprite){
					...
					if(sprite.getSpriteType().equals(SpriteType.BEGIN_POINT) || sprite.getSpriteType().equals(SpriteType.FINISH_POINT) || sprite.getSpriteType().equals(SpriteType.FLOOR) ||sprite.getSpriteType().equals(SpriteType.PLATFORM)){
						if(minX> sprite.getX()+sprite.getWidth()/2){
							minX = sprite.getX()+sprite.getWidth()/2;
						}
						if(maxX < sprite.getX()+sprite.getWidth()/2){
							maxX = sprite.getX()+sprite.getWidth()/2;
						}
					}
					if(sprite.getSpriteType().equals(SpriteType.FLOOR) ||sprite.getSpriteType().equals(SpriteType.PLATFORM) || sprite.getSpriteType().equals(SpriteType.PLATFORM_MOBILE)){
						if(minY> sprite.getY()+sprite.getHeight()/2){
							minY = sprite.getY()+sprite.getHeight()/2;
						}
						if(maxY < sprite.getY()+sprite.getHeight()/2){
							maxY = sprite.getY()+sprite.getHeight()/2;
						}
					}
