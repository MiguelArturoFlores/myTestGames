...
public class LevelDAO extends SQLiteOpenHelper {
	...
	public ArrayList<Level> loadLevelList() {
		...
		try {
			String SQL = " select l.id, l.name, l.avalible, l.stars, l.position, l.facebook, l.minstar from level l where state = 1 order by l.position ";
	...
	public Level loadLevelById(Long idLevel) {
		...
		try {
			String SQL = " select l.id, l.name, l.avalible, l.stars, l.position, l.helppost, l.facebook, l.minstar from level l where state = 1 and l.id = "
	...
	public UserInfo loadUserInfo() {
		...
		try {
			String SQL = " select u.powera, u.powerb, u.powerc, u.powerd, u.money, u.publicity, u.lastDate ,u.contBasic ,u.contBat, u.contHole, u.contArmor, u.contZigZag, u.hasrate, u.haslike, u.hasbuy, u.contpowera, u.contpowerb, u.contpowerc, u.contpowerd, u.lastDateWhatsapp, u.email, u.hasSendMail, u.hasSendTwitter, u.hasSendYoutube "
					+ " , u.lastDateFacebook, u.language, u.contShareWhatsapp,u.contShareFacebook, u.contShareTwitter, u.starRating, u.youtubeToken, u.sendInfo, u.playerselected from userinfo u ";
			SQLiteDatabase db = this.getReadableDatabase();
			if (db == null) {
				return null;
			}
			...
			if (c.moveToFirst()) {
				...
				do {
					...
					userInfo.setPlayerSelect(Integer.valueOf(c.getString(32)));
	...
	public boolean isStarCollected(Long id) {

		try {
			String SQL = " select l.id, l.collected from star l where l.id = "+id.intValue();
			SQLiteDatabase db = this.getReadableDatabase();
			if (db == null) {
				return true;
			}
			...
			if (c.moveToFirst()) {
				// Recorremos el cursor hasta que no haya más registros
				do {
					if(Integer.valueOf(c.getString(1)) == 1 ){
						return true;
					}else if(Integer.valueOf(c.getString(1)).intValue() == 0 ){
						return false;
					}
	...
	public void collectStar(Long id) {
		SQLiteDatabase db = this.getReadableDatabase();
		try {

			String SQL = " update star set collected = 1 where id = "+ id.intValue();
			// Cursor c = this.getReadableDatabase().rawQuery(SQL, null);
			db.execSQL(SQL);
