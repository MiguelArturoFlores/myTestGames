...
//	}
	...
	public ArrayList<Item> loadItemByIdOwner(Long idOwner) {
		try {
			String SQL = " select io.id, i.name, i.textureName, i.totalHp, i.totalMp, i.level, i.defense, i.attack, i.magicPower, i.consumable, i.itemType, io.equiped, io.idOwner from Item i, ItemOwner io where io.idOwner = "+idOwner+" and io.idItem = i.id ";
			SQLiteDatabase db = this.getReadableDatabase();
			if (db == null) {
				return null;
			}
			...
			if (c.moveToFirst()) {
				...
				do {
					...
					item.setId(Long.valueOf(c.getString(0)));
					item.setName(c.getString(1));
					item.setTextureName(c.getString(2));
					item.setTotalHP(Integer.valueOf(c.getString(3)));
					item.setTotalMP(Integer.valueOf(c.getString(4)));
					item.setLevel(Integer.valueOf(c.getString(5)));
					item.setDefense(Integer.valueOf(c.getString(6)));
					item.setAttack(Integer.valueOf(c.getString(7)));
					item.setMagicPower(Integer.valueOf(c.getString(8)));
					item.setConsumable(Integer.valueOf(c.getString(9)));
					item.setItemType(Integer.valueOf(c.getString(10)));
					item.setEquiped(Integer.valueOf(c.getString(11)));
					item.setIdOwner(Long.valueOf(c.getString(12)));
					itemList.add(item);
	...
	public PlayerModel loadPlayerById(Long playerZeo) {
		try {
			String SQL = " select p.id, p.name, p.level, p.totalHp,p.currentHp, p.totalMp, p.currentMp, p.range, p.defense, p.attack, p.magicPower, p.textureName, p.experience , p.totalExperience from playerModel p where p.id = "+playerZeo;
			SQLiteDatabase db = this.getReadableDatabase();
			if (db == null) {
				return null;
			}
			...
			if (c.moveToFirst()) {
				...
				do {
					...
					player.setId(Long.valueOf(c.getString(0)));
					player.setName(c.getString(1));
					player.setLevel(Integer.valueOf(c.getString(2)));
					player.setTotalHP(Integer.valueOf(c.getString(3)));
					player.setCurrentHP(Integer.valueOf(c.getString(4)));
					
					player.setTotalMP(Integer.valueOf(c.getString(5)));
					player.setCurrentMP(Integer.valueOf(c.getString(6)));
					
					player.setDistanceToAttack(Integer.valueOf(c.getString(7)));
					player.setDefense(Integer.valueOf(c.getString(8)));
					player.setAttack(Integer.valueOf(c.getString(9)));
					player.setMagicPower(Integer.valueOf(c.getString(10)));
					player.setTextureName((c.getString(11)));

					player.setCurrentExperience(Integer.valueOf(c.getString(12)));
					player.setExperienceNextLevel(Integer.valueOf(c.getString(13)));
	...
	public void updatePlayerModel(PlayerModel player) {
		SQLiteDatabase db = this.getReadableDatabase();
		try {
			
			String SQL = " update PlayerModel set attack = " +player.getAttack();
			SQL = SQL + " , defense = " +player.getDefense();
			SQL = SQL + " , magicPower= " +player.getMagicPower();
			SQL = SQL + " , level= " +player.getLevel();
			SQL = SQL + " , totalHp= " +player.getTotalHP();
			SQL = SQL + " , currentHp= " +player.getCurrentHP();
			SQL = SQL + " , currentMp= " +player.getCurrentMP();
			SQL = SQL + " , totalMp= " +player.getTotalMP();
			SQL = SQL + " , range = " +player.getDistanceToAttack();
			//SQL = SQL + " , textureName = " +player.getTextureName();
			SQL = SQL + " , experience = " +player.getCurrentExperience();
			SQL = SQL + " , totalExperience = " +player.getExperienceNextLevel();
	...
	public void pickUpItem(Item item) {
		...
		try {
			...
			String SQL = " update ItemOwner set idOwner = " +GameConstants.ID_PLAYER_OWNER +" where id = "+item.getId(); 
	...
	public void dropItem(Item item) {
		...
		try {
			...
			String SQL = " update ItemOwner set idOwner = " +GameConstants.NONE +" where id = "+item.getId(); 
	...
	public ArrayList<PlayerTechnique> loadSpecialTechniqueAvalible(Long id) {
		try {
			String SQL = " select t.idPlayer, t.idTechnique, t.avalible from PlayerTechnique t where t.avalible = 1 ";
			SQLiteDatabase db = this.getReadableDatabase();
			if (db == null) {
				return null;
			}
			...
			ArrayList<PlayerTechnique> techniqueList = new ArrayList<PlayerTechnique>();
			PlayerTechnique technique = new PlayerTechnique();
			// Nos aseguramos de que existe al menos un registro
			if (c.moveToFirst()) {
				// Recorremos el cursor hasta que no haya más registros
				do {
					technique = new PlayerTechnique();

					technique.setPlayerId(Long.valueOf(c.getString(0)));
					technique.setTechniqueId(Long.valueOf(c.getString(1)));
					technique.setAvalible((Integer.valueOf(c.getString(2)).equals(
							Integer.valueOf(1)) ? true : false));
	...
	public Item loadItemById(int idItem) {
		try {
			String SQL = " select i.id, i.name, i.textureName, i.totalHp, i.totalMp, i.level, i.defense, i.attack, i.magicPower, i.consumable, i.itemType from Item i where i.id = "+idItem;
			SQLiteDatabase db = this.getReadableDatabase();
			if (db == null) {
				return null;
			}
	...
	public void unlockPlayerTechnique(int idPlayer, int idTechnique) {
		SQLiteDatabase db = this.getReadableDatabase();
		try {
			
			String SQL = " update PlayerTechnique set avalible = " +1+" where idPlayer = "+idPlayer+" and idTechnique = "+idTechnique; 
