...
public class SpriteEnemy extends MyAnimateSprite {
	...
	public void updateAnimated(float dTime, LevelController level) {
		try {
			...
			float minX = camera.getCenterX() - MainDropActivity.CAMERA_WIDTH
					/ 2;
			float maxX = camera.getCenterX() + MainDropActivity.CAMERA_WIDTH
					/ 2;

			float minY = camera.getCenterY() - MainDropActivity.CAMERA_HEIGHT
					/ 2;
			float maxY = camera.getCenterY() + MainDropActivity.CAMERA_HEIGHT
					/ 2;

			if (this.getX() < minX - 50 || this.getX() > maxX + 50
					|| this.getY() < minY - 50 || this.getY() > maxY + 50) {
				return;
			}
	...
	private void updateMoving(float dTime, LevelController level) {
		try {
			...
			if (!lookingPlayer) {
				...
				Point playerP = new Point(level.getPlayerAdventure().getX(),
						level.getPlayerAdventure().getY());
	...
	private boolean searchForPlayer(LevelController level) {
		try {
			...
			Point playerP = new Point(level.getPlayerAdventure().getX(), level
					.getPlayerAdventure().getY());
	...
	public void generatePathToPoint(Point pointToMove) {
		try {
			...
			path = level.getLevelWorld().calculatePath(
					new Point(this.getX() + this.getWidth() / 2, this.getY()
							+ this.getHeight() / 2), pointToMove, 400);
	...
	public void setXmlParameter(String parameter) {
		try {
			...
			for (Long param : parameterList) {
				try {
					if (i == 0) {
						this.attack = param.intValue();
					}
					if (i == 1) {
						this.defense = param.intValue();
					}
					if (i == 2) {
						this.HP = param.intValue();
					}
					if (i == 3) {
						this.experience = param.intValue();
					}
