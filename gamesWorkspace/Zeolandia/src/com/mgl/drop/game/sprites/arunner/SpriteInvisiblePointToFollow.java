...
public class SpriteInvisiblePointToFollow extends MySprite {
	...
	public void update(float dTime, LevelController level) {
		...
		try {
			...
			float playerX = level.getPlayerAdventure().getX()
					+ level.getPlayerAdventure().getWidth();
			float playerY = level.getPlayerAdventure().getY()
					+ level.getPlayerAdventure().getHeight()/2;
	...
	private void validateBorders(LevelController level) {
		try {
			...
			maxX = level.getLevelWorld().getWidthInPixel();
			maxY = level.getLevelWorld().getHeightInPixel();
