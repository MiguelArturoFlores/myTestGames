...
public class EntityDrawWorld extends MyEntity {
	...
	private void drawEnemies(LevelController level) {
		try {
			ArrayList<SpriteEnemyBattle> enemyBattle = new ArrayList<SpriteEnemyBattle>();
			for (MySpriteGeneral spr : level.getSpriteList()) {
				if (spr instanceof SpriteEnemyBattle) {
					...
					enemyBattle.add(enemy);

				}
			}

			TextureSingleton texture = TextureSingleton.getInstance();
			BattleWorld levelBattleWorld = level.getLevelBattleWorld();

			for (SpriteEnemyBattle enemy : enemyBattle) {

				float minDistance = Float.MAX_VALUE;

				for (DrawNode nodeD : list) {
					SpriteBackground node = nodeD.getHexagon();
					if (validateCollitionWithPoint(node.getX(),
							node.getWidth(), node.getY(), node.getHeight(),
							new Point(enemy.getX() + enemy.getWidth() / 2,
									enemy.getY() + enemy.getHeight() / 2))) {

						node.setAlpha(1f);
						node.setColor(0 / 255f, 0 / 255f, 255 / 255f);
