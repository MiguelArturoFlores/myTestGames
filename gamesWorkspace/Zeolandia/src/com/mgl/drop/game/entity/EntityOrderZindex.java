...
public class EntityOrderZindex extends MyEntity{
	...
	public void updateChild(float dTime, LevelController level) {
		try {
			...
			float minX = camera.getCenterX()-MainDropActivity.CAMERA_WIDTH/2 -1000;
			float maxX = camera.getCenterX()+MainDropActivity.CAMERA_WIDTH/2 +1000;
			
			float minY = camera.getCenterY()-MainDropActivity.CAMERA_HEIGHT/2 -1000;
			float maxY = camera.getCenterY()+MainDropActivity.CAMERA_HEIGHT/2 +1000;
