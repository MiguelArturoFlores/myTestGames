...
import com.mgl.drop.game.scene.BattleScene;
...
import com.mgl.drop.game.sprites.azeoland.SpriteMapAnimation;
...
public class LevelController {
	...
	public void loadLevel(Long idLevel, Long x, Long y) {
		try {
			...
			loadLevel(idLevel);
			player.setMustRePosition(false);
	...
	private void loadNormal(Long idLevel) {
		try {
			...
			InventarySingleton.getInstance();
			...
			if (checkPoint != null) {
				camera.setCenterDirect(checkPoint.getX() + 200,
						checkPoint.getY());
			}
			...
			SpriteBlackBackground black = new SpriteBlackBackground(0, 0,
			...
			black.setSize(MainDropActivity.CAMERA_WIDTH,
	...
	private void addStaticCollitions() {
		try {

			SpriteCollitionRpg collition = null;
			ArrayList<SpriteCollitionRpg> collitionList = new ArrayList<SpriteCollitionRpg>();

			for (MySpriteGeneral spr : spriteList) {
				try {

					if (spr instanceof SpriteTree) {
						switch (((SpriteTree) spr).getIdCollition()) {
						case SpriteTypeConstant.TREE1:

							collition = (SpriteCollitionRpg) MyFactory
									.createObstacle(
											SpriteTypeConstant.COLLITION1, this);
							collition.setSize(24, 12);
							collition.setPosition(
									((SpriteTree) spr).getX() + 10,
									((SpriteTree) spr).getY() + 24);
							collitionList.add(collition);
						...
						case SpriteTypeConstant.TREE2:
							...
							collition = (SpriteCollitionRpg) MyFactory
									.createObstacle(
											SpriteTypeConstant.COLLITION1, this);
							collition.setSize(108, 12);
							collition.setPosition(
									((SpriteTree) spr).getX() + 2,
									((SpriteTree) spr).getY() + 32);
							collitionList.add(collition);
			...
			spriteList.addAll(collitionList);
	...
	public void updateSelect(float dTime) {
		try {
	...
	private void validateBattleMode() {
		try {
			...
			BattleScene battleScene = new BattleScene(1000L, this);
			SceneManagerSingleton.getInstance().goBattleScene(battleScene);
			isInBattleMode = true;
	...
	public void initGameHUD() {
		try {
			...
			ButtonGeneral buttonMenu = new ButtonGeneral(0, 0,
					texture.getTextureByName("buttonMenu.png"),
					texture.getVertexBufferObjectManager(),
					new ButtonListener() {

						@Override
						public void onActionUp(float x, float y) {
							MenuHUD hud = new MenuHUD(getLevelManager()
									.getLevel());
							HUDManagerSingleton.getInstance().addHUD(hud, true);
							setMustUpdate(false);
	...
	public void loadLevelBattle(Long idLevel, LevelController controller) {
		try {
			...
			initSceneTouchListenerBattle();

			ArrayList<MySpriteGeneral> playerBattleList = loadPlayerBattle();

			ArrayList<MySpriteGeneral> enemyList = loadEnemyBattle(controller);

			enemyList.addAll(playerBattleList);

			combatManager = new EntityCombatManager(this, enemyList);
			spriteList.add(combatManager);
	...
	private ArrayList<MySpriteGeneral> loadEnemyBattle(
			LevelController controller) {
		try {
			ArrayList<MySpriteGeneral> enemyList = new ArrayList<MySpriteGeneral>();

			for (MySpriteGeneral spr : controller.spriteList) {
				if (spr instanceof MyAnimateSprite
						&& spr.getSpriteType().equals(SpriteType.ENEMY)) {
					MyAnimateSprite enemy = (MyAnimateSprite) spr;

					float distance = Point.distanceBetween(
							new Point(controller.player.getX(),
									controller.player.getY()),
							new Point(enemy.getX(), enemy.getY()));
					if (distance < EntityCombatManager.DISTANCE_TO_BATTLE) {
						enemy.detachSelf();
						controller.removeEntity(spr);
						SpriteEnemyBattle enemyBattle = (SpriteEnemyBattle) MyFactory
								.createObstacle(
										SpriteTypeConstant.ENEMY_BATTLE, this);
						try {
							SpriteEnemy enemyModel = (SpriteEnemy) spr;
							enemyBattle.setAtts(enemyModel);
							enemyList.add(enemyBattle);
						} catch (Exception e) {
							e.printStackTrace();
						}
		...
		return new ArrayList<MySpriteGeneral>();
	}

	private ArrayList<MySpriteGeneral> loadPlayerBattle() {
		try {

			ArrayList<MySpriteGeneral> playerList = new ArrayList<MySpriteGeneral>();

			for (int i = 0; i < 1; i++) {
				SpritePlayerBattle player = MyFactory.createPlayerBattle(this);
				player.setCharacterFace("zeoFace.png");
	...
	public void manageSceneTouchBattle(float x, float y) {
		try {

			combatManager.manageSceneTouch(x, y);
	...
	private void validateGivePotion() {
		try {
			
			if(PlayerSingleton.getInstance().isGiveLifePotion()){
				
				LevelDAO dao = new LevelDAO(SceneManagerSingleton.getInstance()
						.getActivity(), DatabaseDrop.DB_NAME, null,
						MainDropActivity.DB_VERSION);
				
				Item item = dao.loadItemById(Item.HEALTH_POTION);
				item.setId(InventarySingleton.ID_HEALTH_POTION_FREE);
				
				SpriteChestAdventure chest  = MyFactory.createChest(item, this);
				addSpriteToUpdate(chest);

				chest.setPosition(player.getX(),player.getY()+player.getHeight()+5);
				
				scene.attachChild(chest);
				
				PlayerSingleton.getInstance().setGiveLifePotion(false);
			}
			
			if(PlayerSingleton.getInstance().isGiveManaPotion()){
				
				LevelDAO dao = new LevelDAO(SceneManagerSingleton.getInstance()
						.getActivity(), DatabaseDrop.DB_NAME, null,
						MainDropActivity.DB_VERSION);
				
				Item item = dao.loadItemById(Item.MANA_POTION);
				item.setId(InventarySingleton.ID_HEALTH_POTION_FREE);
				...
				PlayerSingleton.getInstance().setGiveManaPotion(false);
			}
	...
	public EntityGenerateLevelInfinite getInfiniteLevel() {
		return infiniteLevel;
	}

	public void setInfiniteLevel(EntityGenerateLevelInfinite infiniteLevel) {
	...
	public void showCombatHud(SpritePlayerBattle playerBattle,
			EntityCombatManager combatManager) {
		try {

			combatManager.setCurrentPlayer(playerBattle);

			if (actionHUD != null) {
				actionHUD.removeHUD();
			}

			actionHUD = new ActionHUD(playerBattle, combatManager);
			HUDManagerSingleton.getInstance().addHUD(actionHUD, false);
	...
	public void createSelecLevel(SceneSelectLevel sceneSelectLevel) {
		try {
			...
			SpriteMapAnimation map = new SpriteMapAnimation(0, 0, texture.getTextureByName("goldenHead.png"),texture.getVertexBufferObjectManager(), this, sceneSelectLevel);
			map.setSize(MainDropActivity.CAMERA_WIDTH, MainDropActivity.CAMERA_HEIGHT);
			map.setPosition(0,0);
			spriteList.add(map);
