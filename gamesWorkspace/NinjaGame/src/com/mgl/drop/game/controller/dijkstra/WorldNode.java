...
//	private ArrayList<WorldNode> nodeList = new ArrayList<WorldNode>();
	...
	private boolean softCollition = false;

	private boolean corriged = false;
	...
	public boolean isOccupedForVision() {
		
		try {
			
			if(!isSoftCollition() && state.equals(OCCUPED)){
				return true;
			}
			return false;
		} catch (Exception e) {
	...
	public boolean isSoftCollition() {
		return softCollition;
	}

	public void setSoftCollition(boolean softCollition) {
		this.softCollition = softCollition;
	}
