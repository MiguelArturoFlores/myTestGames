...
public class LevelWorld {
	...
	public LevelWorld cloneWorld(){
		try {
			
			LevelWorld level = new LevelWorld();
			
			level.width = width;
			level.height = height;
			level.boxWidth = boxWidth;
			level.boxHeight = boxHeight;

			level.worldNode = new WorldNode[width][height];
			level.generateWorld();
			
			for (int j = 0; j < height; j++) {
				for (int i = 0; i < width; i++) {

					level.worldNode[i][j].setState(worldNode[i][j].getState());
				...
				for (int i = x; i < x + width; i++) {
					...
					if (node.getX() + boxWidth / 2 >= spr.getX()
						...
						if (node.getY() + boxHeight / 2 >= spr.getY()
							...
							node.setSoftCollition(spr.isSoftCollition());
						}
					}
	...
	public Stack<Point> calculatePath(Point sourcePoint, Point targetPoint,
		...
		try {
			...
			System.out.println("LOOKING FOR PATH LEVEL WORLD 312");
	...
	public WorldNode getBox(float x, float y) {
		try {

			int xBegin = (int) ((x / boxWidth));
			int yBegin = (int) ((y / boxHeight));
	...
	public WorldNode getRight(float x, float y) {
		try {
	...
	public WorldNode getLeft(float x, float y) {
		try {
	...
	public WorldNode getUp(float x, float y) {
		try {
	...
	public WorldNode getDown(float x, float y) {
		try {
