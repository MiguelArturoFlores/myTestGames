...
import com.mgl.drop.game.entity.aninja.EntityNinjaFade;
...
import com.mgl.drop.game.sprites.button.aninja.ButtonAction;
import com.mgl.drop.game.sprites.button.aninja.ButtonWhisthle;
...
public class LevelController {
	...
	//change level 
	boolean loadOtherLevel = false;
	Long idLevelToLoad = null;
	float xPlayer = 0;
	float yPlayer = 0;
	...
	private void loadNormal(Long idLevel) {
		try {
			...
			idLevel = Long.valueOf(UserInfoSingleton.getInstance()
					.getCurrentLevel());
			idLevel = 10l;
	...
	private void initEntityNinjaFade() {
		try {
			
			EntityNinjaFade fadeninja = new EntityNinjaFade(scene);
			spriteList.add(fadeninja);
	...
	private void initEntityAddSprite() {
		try {

			EntityAddSpriteImage entity = new EntityAddSpriteImage(spriteList);
	...
	private void initLevelWorld() {
		try {
			...
			levelWorld = new LevelWorld(x, y, 50, 50);
		...
		try {
			...
			for (MySpriteGeneral spr : spriteList) {
				if (spr instanceof SpritePlayer) {
					this.player = (SpritePlayer) spr;
					break;
				}
			}

			// EntityDrawWorld draw = new EntityDrawWorld(this);
			// spriteList.add(draw);
	...
	private void validateChangeLevel() {
		try {
	...
	public void winLevel(Long idLevel) {
		try {

			UserInfoSingleton.getInstance().setCurrentLevel(idLevel.intValue());
			loadLevel(idLevel, 0l, 0l);
	...
	public void initGameHUD() {
		try {
			...
			SpriteBackground background = new SpriteBackground(0, 0,
					...
					texture.getVertexBufferObjectManager());
			//HUDManagerSingleton.getInstance().getTop().attachChild(background);

			ButtonAction action = new ButtonAction(0, 0,
					texture.getTextureByName("actionButton.png"),
					texture.getVertexBufferObjectManager(), this);
			action.setPosition(
					MainDropActivity.CAMERA_WIDTH - action.getWidth(),
					MainDropActivity.CAMERA_HEIGHT - action.getHeight());

			//HUDManagerSingleton.getInstance().getTop().attachChild(action);
			//HUDManagerSingleton.getInstance().getTop()
			//		.registerTouchArea(action);

			ButtonWhisthle wisth = new ButtonWhisthle(0, 0,
					texture.getTextureByName("wisthleButton.png"),
					texture.getVertexBufferObjectManager(), this);
			wisth.setSize(50, 50);
			wisth.setPosition(MainDropActivity.CAMERA_WIDTH - action.getWidth(),
					MainDropActivity.CAMERA_HEIGHT - action.getHeight());
			HUDManagerSingleton.getInstance().getTop().attachChild(wisth);
			HUDManagerSingleton.getInstance().getTop().registerTouchArea(wisth);
	...
	public SpriteVictory getVictory() {
		return victory;
	}

	public void setVictory(SpriteVictory victory) {
